...
final class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('local_iec_project', [
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        $contextlist->add_from_sql(
            'SELECT DISTINCT contextid FROM {local_iec_project} WHERE ownerid = :ownerid',
            ['ownerid' => $userid]
        );
        $contextlist->add_from_sql(
        ...
        $contextlist->add_from_sql(
            'SELECT DISTINCT p.contextid
               FROM {local_iec_revision} r
               JOIN {local_iec_project} p ON p.id = r.projectid
    ...
    public static function get_users_in_context(\core_privacy\local\request\userlist $userlist): void {
        ...
        $userlist->add_from_sql('ownerid', 'SELECT ownerid FROM {local_iec_project} WHERE contextid = :contextid', [
            'contextid' => $contextid,
        ]);
    ...
    public static function export_user_data(approved_contextlist $contextlist): void {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            $projects = $DB->get_records('local_iec_project', ['contextid' => $context->id, 'ownerid' => $userid]);
            foreach ($projects as $project) {
                $data = (object) [
                    ...
                    'revisions' => array_values($DB->get_records('local_iec_revision', [
                ...
                $publications = $DB->get_records('local_iec_publication', ['projectid' => $project->id]);
    ...
    private static function delete_projects(int $contextid, ?int $ownerid): void {
        ...
        $projects = $DB->get_records('local_iec_project', $conditions, '', 'id');
        if (!$projects) {
            return;
        }
        $projectids = array_keys($projects);
        [$insql, $params] = $DB->get_in_or_equal($projectids, SQL_PARAMS_NAMED);
        $publications = $DB->get_records_select('local_iec_publication', "projectid {$insql}", $params, '', 'id');
        $DB->delete_records_select('local_iec_publication', "projectid {$insql}", $params);
        $DB->delete_records_select('local_iec_revision', "projectid {$insql}", $params);
        $DB->delete_records_select('local_iec_project', "id {$insql}", $params);
